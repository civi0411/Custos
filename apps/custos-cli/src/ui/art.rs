...
//!
//! All public functions use **fully dynamic responsive layout**:
//! - Dynamic terminal width detection (live console query + COLUMNS env support)
//! - Proportional owl scaling (scales up gracefully on wide monitors, scales down smoothly on small screens)
//! - Tiered responsive layouts (side-by-side on wide screens, elegant stacked on compact windows)
//! - Zero line-wrapping guarantees across all terminal sizes

use crate::ui::{get_terminal_width, OperationalMode, ResponsiveTier};
use super::assets::{AssetKind, render_pixel_art_to_lines};
...
/// Compute the optimal owl pixel-column width for a 3-owl side-by-side layout.
///
/// Layout anatomy (all in terminal columns):
///   `"  "` (2) + `left_margin` + owl + `gap` + owl + `gap` + owl + `right_margin`
///
/// Safely reserves 2 margin columns at the right edge so Windows console buffers
/// never trigger automatic line wrapping.
///
/// Returns `(owl_w_px, owl_h_px, gap, left_margin)` all in terminal/pixel cols.
fn compute_showcase_dims(term_w: usize) -> (u32, u32, usize, usize) {
    ...
    // Friendly, moderate CLI size: max 22 cols wide (12-13 terminal lines tall)
...
/// Render the project mascot (`owl.png`) standing beside the ANSI Shadow Custos banner.
///
/// Fully responsive:
/// - Wide / Ultra-wide (≥ 105 cols): Side-by-side — mascot left, banner text right.
/// - Standard (70..104 cols): Stacked — mascot centered above full banner.
/// - Compact (< 70 cols): Stacked — smaller mascot, clean compact text wordmark without wrapping.
pub fn print_main_owl_banner(version: &str) {
    ...
    match tier {
        ResponsiveTier::UltraWide | ResponsiveTier::Wide => {
            // Dynamic mascot size: ~35-40% of terminal width
            ...
            let left_pad_n = if term_w > combined_w { ((term_w - combined_w) / 2).min(24) } else { 1 };
            ...
            for i in 0..max_lines {
                let left  = if i < mascot_lines.len() { &mascot_lines[i] } else { &blank_mascot };
                let right = if i < banner_lines.len()  { &banner_lines[i] } else { "" };
                println!("{}{}   {}", pad_str, left, right);
            }
            println!();
        }
        ResponsiveTier::Standard => {
            // Stacked layout with full banner
        ...
        ResponsiveTier::Compact => {
            // Narrow / Split screen: Compact stacked layout with zero wrapping
            ...
            println!();
            println!("  \x1b[38;2;251;191;36;1m❄ Custos\x1b[0m \x1b[38;2;148;163;184mv{}\x1b[0m", version);
...
/// Print the 3 integrated asset owls: Coder, Inspector, Steward.
///
/// **Responsive Layout**:
/// - Standard / Wide / Ultra-wide (≥ 58 cols): Dynamic side-by-side layout with owls scaled to fill space.
/// - Narrow / Compact (< 58 cols): Clean vertical stacked mode card with zero text overflow.
pub fn print_modes_showcase() {
    ...
    if term_w >= 58 {
        ...
        // Render owls with crisp pixel-art feature-preserving pipeline
        ...
        for i in 0..max_h {
            ...
            let r = if i < inspector.len() { &inspector[i] } else { &blank };
            let a = if i < steward.len()   { &steward[i] }   else { &blank };
            println!("  {}{}{}{}{}{}", margin_str, c, gap_str, r, gap_str, a);
        }

        println!("  \x1b[38;2;71;85;105m{}\x1b[0m", "─".repeat(divider_len));

        // Labels adapt length based on owl_w
        ...
        let (s1_label, s2_label, s3_label) = if owl_w >= 22 {
            ("Lập trình & Mã nguồn", "Điều tra & Kiểm thử", "Trợ lý & Điều phối")
        } else if owl_w >= 16 {
        ...
        // Ultra-compact stacked list for narrow screens (< 58 cols)
...
/// Print the activated mode card with a crisp owl asset and a welcoming dialogue card.
///
/// **Responsive layout**:
/// - Side-by-side (≥ 70 cols): owl occupies `term_w / 4` columns (max 48), dialogue box fills rest.
/// - Stacked (< 70 cols): owl centered at top, dialogue card below matching terminal width.
pub fn print_mode_card(mode: OperationalMode) {
    ...
    if term_w >= 70 {
        // ── Side-by-side: owl left, dialogue box right ─────────────────────────
        // Friendly CLI size: 20 cols wide, 24 pixels high = 12 terminal lines (matches dialogue box exactly)
        let owl_w_px = 20u32;
        let owl_h_px = 24u32;
        ...
        // ── Stacked layout for narrow / split windows (< 70 cols) ───────────────
...
mod tests {
    ...
    fn test_print_mascot_sizes() {
        ...
        for (w, h) in [(12, 12), (14, 16), (16, 20)] {
            println!("\n=== Banner test: Mascot w={} h={} ({} lines) ===", w, h, h / 2);
            ...
            for i in 0..max_l {
                let m = if i < mascot_lines.len() { &mascot_lines[i] } else { &blank };
                let b = if i < banner_lines.len() { &banner_lines[i] } else { "" };
                println!("  {}   {}", m, b);
            }
        }
    }
    ...
    fn test_compute_showcase_dims_no_overflow() {
        // At any terminal width in [40, 300], total layout must never exceed terminal width
        for term_w in [40usize, 60, 75, 80, 90, 100, 120, 150, 180, 200, 220, 260, 300] {
